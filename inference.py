...
def inference():
    ...
    dataloader = create_dataloader(config, dataset, batch_size)

    path_to_save = Path.cwd() / 'results' / 'transformed' / datetime.now().strftime("%Y%m%d_h%Hm%Ms%S")
    path_to_save.mkdir(parents=True, exist_ok=True)

    run_model(model, dataloader, path_to_save, batch_size, fmt_of_output)



def create_dataloader(config, dataset, batch_size=1):
...
def run_model(model, dataloader, path_to_save, batch_size=100, fmt_of_output="npz"):
